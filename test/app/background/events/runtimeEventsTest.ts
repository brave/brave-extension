...
describe('runtimeEvents events', () => {
  describe('chrome.runtime.onStartup listener', () => {
    ...
    let windowCreatedSpy: any
    let tabCreatedSpy: any
    let windowGetAllSpy: any
    beforeAll((cb) => {
    ...
    it.skip('calls windowActions.windowCreated for each window', (cb) => {
      expect.assertions(5)
      p.then((inputWindows) => {
        ...
        expect(windowCreatedSpy.mock.calls[1][0].length).toBe(1)
    ...
    it.skip('calls tabActions.tabCreated for each tab in each window', (cb) => {
      expect.assertions(6)
      p.then((inputWindows) => {
        expect(tabCreatedSpy.mock.calls.length).toBe(3)
        expect(tabCreatedSpy.mock.calls[0][0].length).toBe(1)
